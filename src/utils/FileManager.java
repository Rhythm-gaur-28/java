...
public class FileManager {
    ...
    public static void saveGuestsToFile(GuestLinkedList guestList, String filename) {
        try (PrintWriter writer = new PrintWriter(new FileWriter(filename))) {
            ...
            while (current != null) {
                writer.printf("| %-2d | %-20s | %-23s |\n", current.data.getId(), current.data.getName(),
    ...
    public static void saveRoomsToFile(RoomLinkedList rooms, String filename) {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(filename))) {
            ...
            while (current != null) {
                writer.printf("| %-6d | %-10s | %-7.2f |\n", current.data.getRoomNumber(), current.data.getRoomType(),
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(filename))) {
            ...
            while (current != null) {
                writer.printf("| %-9d | %-7d | %-10d | %-11s | %-11s |\n",
                        current.data.getBookingId(), current.data.getGuestId(), current.data.getRoomNumber(),
                        current.data.getCheckInDate(), current.data.getCheckOutDate());
                current = current.next;
            }
            writer.println("+-----------+---------+------------+-------------+-------------+");
        } catch (IOException e) {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(filename))) {
            ...
            while (current != null) {
                writer.printf("| %-9d | %-17s | %-10d | %-14s |\n",
                        current.data.getRequestId(), current.data.getRequestType(), current.data.getRoomNumber(),
                        current.data.getRequestTime());
                current = current.next;
            }
            writer.println("+-----------+-------------------+------------+----------------+");
